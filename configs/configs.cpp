...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.legitbot.backtrackl, false, crypt_str("Legitbot.backtrackl"));
	setup_item(&g_cfg.legitbot.legit_resolver, false, crypt_str("Legitbot.legit_resolver"));
	...
	for (auto i = 0; i < 5; i++)
	{
		...
		setup_item(&g_cfg.legitbot.weapon[i].backtrack_ticks, 0, std::to_string(i) + crypt_str("Legitbot.backtrack_ticks"));
	...
	setup_item(&g_cfg.menu.menu_theme, Color(255, 0, 0), crypt_str("Menu.menu_color"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\KitsuneSquad\\Configs\\") + config;
		}
